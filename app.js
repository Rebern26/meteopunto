...
// La chiave API non è più nel browser: le richieste passano dalla
// Pages Function su /api/owm, che aggiunge la chiave lato server.
...
async function getCoords(city) {
  const url = `${API_BASE}?type=geo&q=${encodeURIComponent(city)}`;
...
async function fetchCurrent(city) {
  ...
  const url = `${API_BASE}?type=weather&lat=${lat}&lon=${lon}`;
  const res = await fetch(url);
...
async function fetchForecast(city) {
  const { lat, lon } = await getCoords(city);
  const url = `${API_BASE}?type=forecast&lat=${lat}&lon=${lon}`;
  const res = await fetch(url);
